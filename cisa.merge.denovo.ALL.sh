...
if [ -d ${i}_mergeALL ] # if it is a dir ( -d checks if $i is a dir)
then
echo "Changing to ${i}_mergeALL..."
folder=${i}_mergeALL
cd ${i}_mergeALL
else
echo "${i}_mergeALL doesn't exist.  Creating..."
mkdir ${i}_mergeALL
folder=${i}_mergeALL
cd ${i}_mergeALL
fi # if dir check
...
#echo "mira_ref assembly is $mira_ref"
	...
	if [ -f $folder.MERGE.denovo.config ] # if the cisa merge already happened then we don't recreate the script
		...
		echo "count=7" >>$folder.MERGE.denovo.config # tells cisa how many files we'll be merging
