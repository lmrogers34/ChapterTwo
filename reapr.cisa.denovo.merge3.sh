...
perl /cib/lisa/denovo/paired/scripts/contig.truncator.pl -i ${i}.denovo.merge3.cisa.fa -l 500 -o ${i}.denovo.merge3.cisa.fa.truncated.fa
if [ -s ${i}.denovo.merge3.cisa.fa.truncated.fa ] # if the file exists and is not empty
then
	...
	then
		...
		start=`date +%s`
		reapr pipeline ${i}.denovo.merge3.cisa.fa.facheck.fa ${i}_same_map.sorted.bam ${i}-same-reapr
		end=`date +%s`
		#pipleline
		mv *-reapr /cib/lisa/new_l100_paper1/merged_reapr
		runtime=$((end-start))
		echo "Reapr runtime for ${i} is $runtime"
	else # do whole pipeline, first run through
		echo "Beginning pipeline for ${i}"
		# step 1 facheck
		reapr facheck ${i}.denovo.merge3.cisa.fa ${i}.denovo.merge3.cisa.fa.facheck
		# step 1.1 check is facheck was succesful, if it was continuw with fachecked file, if not use original.
	
		if [ -s ${i}.denovo.merge3.cisa.fa.facheck.fa ] # if file exists and is not 0, ie empty # facehck if 1/3
		then
			#2 index
			echo "beginning smalt indexing on ${i}"
			# smalt index -k hashlength -s steplength ref_name contigfile
			smalt index -k 13 -s 2 ${i}_same_index ${i}.denovo.merge3.cisa.fa.facheck.fa
			#3 sample (optional)
		...
		elif [ -s ${i}.denovo.merge3.cisa.fa ] # ie facheck didn't work # facehck if else 2/3
		then
			#2 index
			echo "beginning smalt indexing on ${i}"
			# smalt index -k hashlength -s steplength ref_name contigfile
			smalt index -k 13 -s 2 ${i}_same_index ${i}.denovo.merge3.cisa.fa
			#3 sample (optional)
			echo "beginning smalt sampling on ${i}"
			smalt sample -u 1000 -o ${i}_same_sample -n 24 ${i}_same_index $read1 $read2 
			#4. Map
			...
			then
				...
				echo "beginning reapr pipeline for ${i}"
				start=`date +%s`
				reapr pipeline ${i}.denovo.merge3.cisa.fa ${i}_same_map.sorted.bam ${i}-same-reapr
				end=`date +%s`
				mv *-reapr /cib/lisa/new_l100_paper1/merged_reapr
				runtime=$((end-start))
				echo "Reapr runtime for $i is $runtime"
