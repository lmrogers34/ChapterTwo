...
#Start of mira
echo "Beginning Mira reapr assessment on $folder"
cd mira
perl /home/lrogers/work/deNovo-stats/assembly/paired/contig.truncator.pl -i ${folder}_assembly/${folder}_d_results/${folder}_out.unpadded.fasta -l 500 -o ${folder}_assembly/${folder}_d_results/${folder}_out.unpadded.fasta.truncated.fa
if [ -s ${folder}_assembly/${folder}_d_results/${folder}_out.unpadded.fasta.truncated.fa ] # if the file exists and is not empty
then
	...
	then
		...
		start=`date +%s`
		reapr pipeline ${folder}_assembly/${folder}_d_results/${folder}_out.unpadded.fasta.facheck.fa ${folder}_same_map.sorted.bam $folder-mira-same-reapr
		end=`date +%s`
		#pipleline
		...
		# step 1 facheck
		reapr facheck ${folder}_assembly/${folder}_d_results/${folder}_out.unpadded.fasta ${folder}_assembly/${folder}_d_results/${folder}_out.unpadded.fasta.facheck.fa
		# step 1.1 check is facheck was succesful, if it was continuw with fachecked file, if not use original.
	
		if [ -s ${folder}_assembly/${folder}_d_results/${folder}_out.unpadded.fasta.facheck.fa ] # if file exists and is not 0, ie empty # facehck if 1/3
		then
			#2 index
			echo "beginning smalt indexing on $folder"
			# smalt index -k hashlength -s steplength ref_name contigfile
			smalt index -k 13 -s 2 ${folder}_same_index ${folder}_assembly/${folder}_d_results/${folder}_out.unpadded.fasta.facheck.fa 
			#3 sample (optional)
		...
		elif [ -s ${folder}_assembly/${folder}_d_results/${folder}_out.unpadded.fasta ] # ie facheck didn't work # facehck if else 2/3
		then
			#2 index
			echo "beginning smalt indexing on $folder"
			# smalt index -k hashlength -s steplength ref_name contigfile
			smalt index -k 13 -s 2 ${folder}_same_index ${folder}_assembly/${folder}_d_results/${folder}_out.unpadded.fasta
			#3 sample (optional)
			echo "beginning smalt sampling on $folder"
			smalt sample -u 1000 -o ${folder}_same_sample -n 24 ${folder}_same_index $read1 $read2 
			#4. Map
			...
			then
				...
				echo "beginning reapr pipeline for $folder"
				start=`date +%s`
				reapr pipeline ${folder}_assembly/${folder}_d_results/${folder}_out.unpadded.fasta ${folder}_same_map.sorted.bam $folder-mira-same-reapr
				end=`date +%s`
				mv *-reapr /cib/lisa/new_l100_paper1/reapr-same-500bp+
				runtime=$((end-start))
				echo "Reapr runtime for $k is $runtime"
...
#End of mira 
echo "Finished Mira reapr assessment on $folder"
