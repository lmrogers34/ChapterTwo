...
#Start of sga
echo "Beginning sga reapr assessment on $folder"
cd sga

##remove###

#reapr smaltmap ${i}_overlap_${j}-contigs.fa.facheck.fa $read1 $read2 $i-sga-k$j.bam
#pipleline
#reapr pipeline ${i}_overlap_${j}-contigs.fa.facheck.fa $i-sga-k$j.bam $i-sga-k$j-reapr
##remove###
#for j in {21..101..2}
for j in {103..127..2}
do
	#run contig truncator..  then check to see if it is empty
	perl /cib/lisa/denovo/paired/scripts/contig.truncator.pl -i ${i}_overlap_${j}-contigs.fa -l 500 -o ${i}_overlap_${j}-contigs.fa.truncated.fa

	if [ -s ${i}_overlap_${j}-contigs.fa.truncated.fa ] # if the file exists and is not empty
	then
		...
		then
			...
			start=`date +%s`
			reapr pipeline ${i}_overlap_${j}-contigs.fa.facheck.fa ${i}_${j}_same_map.sorted.bam ${i}_k${j}-sga-same-reapr
			end=`date +%s`
			#pipleline
			mv *-reapr /cib/lisa/olc_sga/sga_reapr
			runtime=$((end-start))
			echo "Reapr runtime for ${i}_${j} is $runtime"
		else # do whole pipeline
			# step 1 facheck
			reapr facheck ${i}_overlap_${j}-contigs.fa ${i}_overlap_${j}-contigs.fa.facheck
			# step 1.1 check is facheck was succesful, if it was continuw with fachecked file, if not use original.
	
			if [ -s ${i}_overlap_${j}-contigs.fa.facheck.fa ] # if file exists and is not 0, ie empty # facehck if 1/3
			then
				#2 index
				echo "beginning smalt indexing on $i-$j"
				# smalt index -k hashlength -s steplength ref_name contigfile
				smalt index -k 13 -s 2 ${i}_${j}_same_index ${i}_overlap_${j}-contigs.fa.facheck.fa 
				#3 sample (optional)
				echo "beginning smalt sampling on $i-$j"
				smalt sample -u 1000 -o ${i}_${j}_same_sample -n 2 ${i}_${j}_same_index $read1 $read2 
				#4. Map
				...
				samtools sort ${i}_${j}_same_map.bam -o ${i}_${j}_same_map.sorted.bam
				#7 Reapr pipeline
			...
			elif [ -s ${i}_overlap_${j}-contigs.fa ] # ie facheck didn't work # facehck if else 2/3
			then
				#2 index
				echo "beginning smalt indexing on $i-$j"
				# smalt index -k hashlength -s steplength ref_name contigfile
				smalt index -k 13 -s 2 ${i}_${j}_same_index ${i}_overlap_${j}-contigs.fa 
				#3 sample (optional)
				echo "beginning smalt sampling on $i-$j"
				smalt sample -u 1000 -o ${i}_${j}_same_sample -n 2 ${i}_${j}_same_index $read1 $read2 
				#4. Map
				...
				then
					...
					echo "beginning reapr pipeline for $j"
					start=`date +%s`
					reapr pipeline ${i}_overlap_${j}-contigs.fa ${i}_${j}_same_map.sorted.bam ${i}_k${j}-sga-same-reapr
					end=`date +%s`
					mv *-reapr /cib/lisa/olc_sga/sga_reapr
					runtime=$((end-start))
					echo "Reapr runtime for ${i}_${j} is $runtime"
