...
#Start of velvet
echo "Beginning velvet reapr assessment on $folder"
cd velvet

##remove###

#reapr smaltmap $j/contigs.fa.facheck.fa $read1 $read2 $i-velvet-k$j.bam
#pipleline
#reapr pipeline $j/contigs.fa.facheck.fa $i-velvet-k$j.bam $i-velvet-k$j-reapr
##remove###
for j in {21..99..2}
do
if [ -d "$j" ]
then
	...
	then
		...
		then
			...
			start=`date +%s`
			reapr pipeline $j/contigs.fa.facheck.fa ${i}_${j}_same_map.sorted.bam ${i}_k${j}-velvet-same-reapr
			end=`date +%s`
			#pipleline
			...
			then
				...
				then
					...
					echo "beginning reapr pipeline for $j"
					start=`date +%s`
					reapr pipeline $j/contigs.fa ${i}_${j}_same_map.sorted.bam ${i}_k${j}-velvet-same-reapr
					end=`date +%s`
					mv *-reapr /cib/lisa/new_l100_paper1/reapr-same-500bp+
					runtime=$((end-start))
					echo "Reapr runtime for ${i}_${j} is $runtime"
