...
#Start of velvet_ref
echo "Beginning velvet_ref reapr assessment on $folder"
cd velvet_ref

##remove###

#reapr smaltmap $j/contigs.fa.facheck.fa $read1 $read2 $i-velvet_ref-k$j.bam
#pipleline
#reapr pipeline $j/contigs.fa.facheck.fa $i-velvet_ref-k$j.bam $i-velvet_ref-k$j-reapr
##remove###
for j in {21..127..2}
do
if [ -d "$j" ]
then
	...
	perl /home/lrogers/work/deNovo-stats/assembly/paired/contig.truncator.pl -i $j/contigs.fa -l 500 -o $j/contigs.fa.truncated.fa
	if [ -s $j/contigs.fa.truncated.fa ] # if the file exists and is not empty
	then
		...
		then
			...
			start=`date +%s`
			reapr pipeline $j/contigs.fa.facheck.fa ${i}_${j}_same_map.sorted.bam ${i}_k${j}-velvet_ref-same-reapr
			end=`date +%s`
			#pipleline
			...
			elif [ -s $j/contigs.fa ] # ie facheck didn't work # facehck if else 2/3
			then
				#2 index
				echo "beginning smalt indexing on $i-$j"
				# smalt index -k hashlength -s steplength ref_name contigfile
				smalt index -k 13 -s 2 ${i}_${j}_same_index $j/contigs.fa 
				#3 sample (optional)
				echo "beginning smalt sampling on $i-$j"
				smalt sample -u 1000 -o ${i}_${j}_same_sample -n 24 ${i}_${j}_same_index $read1 $read2 
				#4. Map
				...
				then
					...
					echo "beginning reapr pipeline for $j"
					start=`date +%s`
					reapr pipeline $j/contigs.fa ${i}_${j}_same_map.sorted.bam ${i}_k${j}-velvet_ref-same-reapr
					end=`date +%s`
					mv *-reapr /cib/lisa/new_l100_paper1/reapr-same-500bp+
					runtime=$((end-start))
					echo "Reapr runtime for ${i}_${j} is $runtime"
