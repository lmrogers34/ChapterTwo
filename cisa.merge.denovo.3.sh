...
if [ -d ${i}_merge3 ] # if it is a dir ( -d checks if $i is a dir)
then
echo "Changing to ${i}_merge3..."
folder=${i}_merge3
cd ${i}_merge3
else
echo "${i}_merge3 doesn't exist.  Creating..."
mkdir ${i}_merge3
folder=${i}_merge3
cd ${i}_merge3
fi # if dir check
...
#echo "mira_ref assembly is $mira_ref"
	...
	if [ -f $folder.MERGE.denovo.config ] # if the cisa merge already happened then we don't recreate the script
		...
		echo "count=3" >>$folder.MERGE.denovo.config # tells cisa how many files we'll be merging
