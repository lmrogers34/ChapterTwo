...
if [ -d ${i}_merge6_R2_0.45 ] # if it is a dir ( -d checks if $i is a dir)
then
echo "Changing to ${i}_merge6_R2_0.45..."
folder=${i}_merge6_R2_0.45
cd ${i}_merge6_R2_0.45
else
echo "${i}_merge6_R2_0.45 doesn't exist.  Creating..."
mkdir ${i}_merge6_R2_0.45
folder=${i}_merge6_R2_0.45
cd ${i}_merge6_R2_0.45
fi # if dir check
...
#echo "mira_ref assembly is $mira_ref"
	...
	if [ -f $folder.MERGE.denovo.config ] # if the cisa merge already happened then we don't recreate the script
		...
		echo "data=$velvet_ref,title=velvet_ref" >>$folder.MERGE.denovo.config
#		echo "data=$mira_ref,title=mira_ref" >>$folder.MERGE.denovo.config
...
echo "R2_Gap=0.45" >>$folder.CISA.denovo.config
